...
Release the application runtime and PostgreSQL migration images from one clean
working-tree revision with:
...
tag, builds `Dockerfile` and `Dockerfile.migrations` from the repository root,
verifies the Git commit and working-tree state stayed unchanged between builds,
stamps and validates the same OCI source-revision label on both images,
authenticates Docker using `aws ecr get-login-password`, and pushes:
...
references for both artifacts. It does not create repositories. For a
...
static web assets, and the authoritative runtime contracts at:

- `/app/contracts/policy/policy-schema.v1.json`
- `/app/contracts/proposed-governance-change/proposed-governance-change.v1.schema.json`

It also contains these non-secret or development/sample YAML defaults:
